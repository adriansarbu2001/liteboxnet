...
def test_network_output_shape(device_id=-1):
    ...
    sample_input = torch.randn(1, 3, 352, 1216).float().to(device)
    model = LiteBoxNet(backbone_pretrained=True, am=[PAM, CAM, DAM]).to(device)
    output = model(sample_input)
    print("Output shape:", output.shape)


def test_inference(network, threshold, device_id=-1):
    device = torch.device('cpu' if device_id < 0 else 'cuda:%d' % device_id)
    dataset = LiteBoxNetDataset(
        base_root="D:/UBB/Inteligenta Computationala Aplicata EN/SEM I/datasets/liteboxnet",
        split="testing",
    ...
    image = plot_liteboxnet_label(image, output[0], threshold=threshold)
